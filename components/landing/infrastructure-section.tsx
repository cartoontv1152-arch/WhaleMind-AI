...
const regions = [
  { name: "SoSoValue API", nodes: 12, status: "live" },
  { name: "SoDEX Spot", nodes: 8, status: "live" },
  { name: "ValueChain RPC", nodes: 6, status: "guarded" },
  { name: "AI Risk Layer", nodes: 3, status: "active" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                WhaleMind reads SoSoValue intelligence, prepares SoDEX routes, and anchors execution to ValueChain.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="absolute inset-0 opacity-70">
              ...
              {[...Array(20)].map((_, i) => (
                <div
                  key={i}
                  className="absolute w-1.5 h-1.5 rounded-full bg-whale-accent"
                  style={{
                    left: `${10 + (i % 5) * 20}%`,
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-8xl lg:text-[10rem] font-display leading-none">286623</span>
                <span className="text-2xl text-muted-foreground">chain id</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                ValueChain mainnet identity used for wallet switching, EIP-712 signing, and on-chain verification.
              </p>
            </div>
          </div>
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className={`w-2 h-2 rounded-full transition-colors ${
                  activeRegion === index ? "bg-whale-accent" : "bg-foreground/20"
                }`} />
              ...
              <span className="text-sm text-muted-foreground">{region.nodes} checks</span>
            </div>
          ))}
        </div>
      </div>
